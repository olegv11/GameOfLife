...
{
    // Ячейки храним как битовую карту. 1 бит - 1 ячейка
    ...
    int bytes = (((height + 2) * (width + 2) + 7) / 8) * 8;
    m_stride = width + 2;
...
{
    int bitNumber = (y + 1) * m_stride + (x + 1);
    int byte = bitNumber / 8;
    int shift = bitNumber % 8;

    if (m_field[byte] & (1 << shift))
    {
        return 1;
    }
    else
